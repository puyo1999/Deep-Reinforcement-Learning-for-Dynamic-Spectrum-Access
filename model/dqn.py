...
class DQNetwork:
    def __init__(self, learning_rate, state_size,action_size,hidden_size,step_size, memory, name='DQN'):
    ...
    def learn(self, error):
        print('DQN learn - saving error:{}\n'.format(self.error))
        self.error = error

    def store_transition(self, s, a, r, s_):
        print('DQN StoreTransition - shape of s:{} a:{}\n'.format(np.shape(s), np.shape(a)))
        print('DQN StoreTransition - s:{}\n a:{}\n r:{}\n s_:{}\n'.format(s, a, r, s_))
        transition = np.hstack(
            [list(s[0]), list(s[1]), list(s[2]), list(np.r_[a, r]), list(s_[0]), list(s_[1]), list(s_[2])])
        print('DQN StoreTransition - transition:{}\n'.format(transition))
        # self.memory.store(transition)
        print('DQN StoreTransition - error:{}\n'.format(self.error))
